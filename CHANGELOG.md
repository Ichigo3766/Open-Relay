...
### Improvements
- Added a Share button to the action bar under every completed assistant response

### Bug Fixes
- Fixed images generated by models that return base64 data URIs in the files array not showing in the app
- Fixed "Show follow-up suggestions" setting not hiding already-stored suggestions when turned off
- Fixed tapping citation links and web links in assistant messages doing nothing — taps now correctly open the in-app browser
- Fixed very long assistant messages (a single giant paragraph or flat list) rendering completely blank 
- Fixed very long user messages rendering blank in the chat bubble — long text is now split into chunks to stay within rendering limits
- Fixed hamburger menu button not responding to taps on its center and left side when the drawer was closed
- Fixed newly opened chats landing mid-conversation instead of at the bottom when Markdown finishes rendering after the initial scroll settles
- Fixed a memory leak where notification observers accumulated each time a chat was reconfigured and were never released after closing a chat
- Fixed rendered message text reappearing after the view was reset — the markdown subject no longer replays its last value to new subscribers
- Fix stale loading placeholder when Markdown content changes during parse
- Fixed duplicate message views staying blank when two views requested the same Markdown parse simultaneously — all waiters are now notified when a shared parse finishes
